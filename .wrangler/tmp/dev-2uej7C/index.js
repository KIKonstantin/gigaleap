...
var STALE_MS = 12e4;
var GigaleapServer = class extends Server {
  static {
    __name(this, "GigaleapServer");
  }
  nextSlot = 0;
  players = /* @__PURE__ */ new Map();
  // conn.id -> { slot, hue, name, last, s: [x,y,z,yaw,g,w] | null }
  timer = null;
  findConn(id) {
  ...
  onConnect(conn) {
    ...
    const roster = [...this.players].filter(([id]) => id !== conn.id).map(([id, p]) => ({ id, hue: p.hue, name: p.name, s: p.s }));
...
// .wrangler/tmp/bundle-QVwpDu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-QVwpDu/middleware-loader.entry.ts
